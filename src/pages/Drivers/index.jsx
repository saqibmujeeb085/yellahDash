...
const Drivers = () => {
  ...
  const Currentdata = EmployeeData.slice(firstPostIndex, lastPostIndex);
  ...
  return (
    <div>
      ...
      <div className='px-4 lg:px-8 flex justify-between items-center'>
        <h2 className='md:text-heading text-[40px] leading-heading font-bold'>Drivers</h2>
        <Link to="/Driver/BlockedDrivers" className='text-baseFont underline font-base'>Blocked Drivers</Link>
      </div>
         ...
         id ={item?.id}
          ...
          {toggle && clicked === item?.id && (
             <ul className='absolute min-w-22 bg-white shadow rounded-lg p-1 -top-3 left-5'>
              <li>
                <Link to="/Drivers/DriverDetails"><RemoveRedEyeIcon style={{fontSize: '12px'}} className="text-gray-500"/> <span className='text-sm text-gray-500'>View</span></Link>
              </li>
      ...
      <td className=' text-sm text-center'><div className='relative group text-sm'>
        <MoreVertIcon onClick={(e) => {setToggle(!toggle);setClicked(item.id); }} className="cursor-pointer"/>
         ...
         <div className='mt-10 flex justify-between items-center flex-col gap-3 md:flex-row'>
          <p>Showing {firstPostIndex + 1} to {lastPostIndex} of {EmployeeData.length} entries</p>

          <Pagination
              totalPost = {EmployeeData.length}
