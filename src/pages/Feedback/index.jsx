...
import feeddta from '../../data/feeddta'
...
const Feedback = () => {
  ...
  const [clicked,setClicked] =useState('');
   ...
   const MessageM = [{
     ConformtionM: 'Are you sure you want to Delete this FeedBack',
     SuccessM: 'FeedBack Is deleted Successfuly'
}]
  
  
   const [value, setValue] = useState('');
   const [dataSource, setDataSource] = useState(feeddta);
  ...
  const Currentdata = feeddta.slice(firstPostIndex, lastPostIndex);
  ...
  return (
    <div>
      ...
      <div className='px-4 lg:px-8 flex justify-between items-center'>
        <h2 className='md:text-heading text-[40px] leading-heading font-bold'>Feedback</h2>
      </div>
      <div className='bg-white shadow-2xl mt-12 py-10 px-5 rounded-3xl'>
       ...
       <table className=' w-full mt-10 border-spacing-y-3 border-separate min-w-[45rem]'>
        ...
        <th className='text-start text-base px-4 '>
          ...
          <th className='text-start text-base px-4 '>
           <ImportExportIcon/> Name
          </th>
          <th className='text-start text-base px-4  '>
           <ImportExportIcon/> Type
          </th>
          <th className='text-start text-base px-4  '>
          ...
          {toggle && clicked === item?.id && (
             <ul className='absolute min-w-22 bg-white shadow rounded-lg p-1 -top-3 left-[110px]'>
              <li>
                <Link to="/Feedback/FeedbackDetails"><RemoveRedEyeIcon style={{fontSize: '12px'}} className="text-gray-500"/> <span className='text-sm text-gray-500'>View</span></Link>
              </li>
      ...
      <td className=' text-sm text-center'><div className='relative group text-sm'>
        <MoreVertIcon onClick={(e) => {setToggle(!toggle);setClicked(item.id); }} className="cursor-pointer"/>
         ...
         <ul 
         id ={item?.id}
          className='absolute w-20 bg-white shadow rounded-lg p-1 -top-3 left-[110px] text-start'
         ...
         <div className='mt-10 flex justify-between items-center flex-col gap-3 md:flex-row'>
          <p>Showing {firstPostIndex + 1} to {lastPostIndex} of {feeddta.length} entries</p>

          <Pagination
              totalPost = {feeddta.length}
