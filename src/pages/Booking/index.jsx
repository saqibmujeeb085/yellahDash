...
const Booking = () => {
  ...
  const [clicked,setClicked] =useState('');
   ...
   const [dataSource, setDataSource] = useState(BookingData);
  ...
  const Currentdata = BookingData.slice(firstPostIndex, lastPostIndex);
  
  
  return (
    <div>
  
      <div className='px-4 lg:px-8 flex justify-Start items-center'>
        <h2 className='md:text-heading text-[40px] leading-heading font-bold'>Booking</h2>
      </div>
      <div className='bg-white shadow-2xl mt-12 py-10 px-5 rounded-3xl'>
       ...
       <table className=' w-full mt-10 border-spacing-y-3 border-separate min-w-[45rem]'>
        ...
        <th className='text-start text-base px-4 '>
           <ImportExportIcon/> Trip ID
          </th>
          <th className='text-start text-base px-4 '>
           <ImportExportIcon/> User Name
          </th>
          <th className='text-start text-base px-4  '>
           <ImportExportIcon/> Driver Name
          </th>
          <th className='text-start text-base px-4  '>
        ...
        <p className="cursor-pointer text-baseFont">
       Completed  
       </p>
         </div>
      </td>
      <td className=' text-sm text-center'><div className='relative group text-sm'>
        <MoreVertIcon onClick={(e) => {setToggle(!toggle);setClicked(item.id); }} className="cursor-pointer"/>
         ...
         <ul 
         id ={item?.id}
          className='absolute w-[75px] bg-white shadow rounded-lg p-1 -top-2 left-[100px] text-start'
          
          >
          <li>
                <Link to="/Booking/BooKingDetails"><RemoveRedEyeIcon style={{fontSize: '12px'}} className="text-gray-500"/> <span className='text-sm text-gray-500'>View</span></Link>
              </li>
         </ul>
         )}
        </div></td>
      ...
      <td className=' text-sm text-center'><div className='relative group text-sm'>
        <MoreVertIcon onClick={(e) => {setToggle(!toggle);setClicked(item.id); }} className="cursor-pointer"/>
         ...
         <div className='mt-10 flex justify-between items-center flex-col gap-3 md:flex-row'>
          <p>Showing {firstPostIndex + 1} to {lastPostIndex} of {BookingData.length} entries</p>
