...
import ConfirmationWithSccuess from '../../components/modals/ConfirmationWithSccuess'
...
const Commissions = () => {
  ...
  const [setComm, setSetComm] = useState(false);
 ...
 const MessageM = [{
  ConformtionM: 'Are you sure you want this transaction to be unpaid',
  SuccessM: 'This transaction has been Marked as unpaid'
}]

  const [conM, setConM] = useState(false);
  ...
  return (
    <div>
     {conM && (
      <div>
      {MessageM.map((item, i)=>
     <ConfirmationWithSccuess key={i} con={item.ConformtionM} suc={item.SuccessM}/>
     )}
     </div>
    )}
...
{setComm && (
  <CommissionModal/>
      ...
      <div className='px-4 lg:px-8 flex justify-between items-center'>
        <h2 className=' text-[40px] leading-heading font-bold'>Commissions</h2>
        <p className='bg-gray-900 text-white py-3 px-9 rounded-xl cursor-pointer' onClick={Opensetcomm}>Set Commission Rates</p>
      </div>
             ...
             <td className='text-start text-sm px-4 text-gray-500'><div className='relative group text-sm'>
              ...
              Paid  <ArrowDropDownIcon />
                ...
                id ={item?.id}
                 ...
                 <li>
                   <p><CloseIcon onClick={(e) => {setAct(false);setClicked(item.id); }} style={{fontSize: '12px', cursor: 'pointer'}} className="text-gray-500"/> <span onClick={(e) => {setConM(!conM);setClicked(item.id);}} className='text-sm cursor-pointer text-gray-500'>unPaid</span></p>
                 </li>
                </ul>
                )}
                </div>
      ...
      <td className='text-start text-sm px-4 text-gray-500'><div className='relative group text-sm'>
       ...
       Paid  <ArrowDropDownIcon />
         ...
         id ={item?.id}
          className='absolute w-20 bg-white shadow rounded-lg p-1 -top-1 left-[35px] text-start'>
          <li>
            <p><CloseIcon onClick={(e) => {setAct(false);setClicked(item.id); }} style={{fontSize: '12px', cursor: 'pointer'}} className="text-gray-500"/> <span onClick={(e) => {setConM(!conM);setClicked(item.id);}} className='text-sm text-gray-500 cursor-pointer'>unPaid</span></p>
          </li>
         </ul>
         )}
         </div>
